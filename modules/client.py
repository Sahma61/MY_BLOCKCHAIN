...
def Send(args):
    ...
    balance = 0
    for x in UTXO.keys():
        if UTXO[x].get(node.config["bitcoin_address"]) is None: continue
        if UTXO[x][node.config["bitcoin_address"]] >= (args.value + args.fee): tx_in = []; tx_in.append((x, node.config["bitcoin_address"])); balance = UTXO[x][node.config["bitcoin_address"]]; break
